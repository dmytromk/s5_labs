...
void init_naive(double** A, double** B, double** C, double** A_block, double** B_block, double** C_block, unsigned int* size, unsigned int* tape_len) {
    ...
    */
    *A_block = (double*)malloc((*tape_len * *tape_len) * sizeof(double));
    *B_block = (double*)malloc((*tape_len * *tape_len) * sizeof(double));
    *C_block = (double*)malloc((*tape_len * *tape_len) * sizeof(double));
...
void run_naive(unsigned int matrix_size) {
    double *A, *B, *C, *A_buffer, *B_buffer, *C_buffer;
    unsigned int tape_len;
    double start_time, end_time;

    init_naive(&A, &B, &C, &A_buffer, &B_buffer, &C_buffer, &matrix_size, &tape_len);
    ...
    */
    MPI_Scatter(A, tape_len * matrix_size, MPI_DOUBLE, A_buffer, tape_len * matrix_size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Scatter(B, tape_len * matrix_size, MPI_DOUBLE, B_buffer, tape_len * matrix_size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    for (int p = 0; p < process_num; p++) {
        for (int i = 0; i < tape_len; i++) {
            for (int j = 0; j < matrix_size; j++) {
                for (int k = 0; k < tape_len; k++) {
                    ...
                    */
                    C_buffer[i * matrix_size + j] += A_buffer[prev_data_num * tape_len +
                                                            i * matrix_size + k] * B_buffer[k * matrix_size + j];
                }
            }
        }
        ...
        рангів процесів).
        */
        MPI_Sendrecv_replace(B_buffer, tape_len * matrix_size, MPI_DOUBLE,
    ...
    */
    MPI_Gather(C_buffer, tape_len * matrix_size, MPI_DOUBLE, C, tape_len * matrix_size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
...
//            SquareMatrix::multiply(A, B, checker, matrix_size);
//
//            if (SquareMatrix::areEqual(C, checker, matrix_size)) {
